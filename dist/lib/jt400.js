...
function createInstance(connection, insertListFun, inMemory) {
    const mixinConnection = function (obj, newConn) {
        ...
        obj.batchUpdate = function (sql, paramsList) {
            const jsonParams = JSON.stringify((paramsList || []).map(row => {
    ...
    const jt400 = mixinConnection({
        ...
        defineProgram(opt) {
            const pgm = connection.connection.pgmSync(opt.programName, JSON.stringify(opt.paramsSchema), opt.libraryName || '*LIBL');
